...
community.cl1 <- data.frame(sizes(fc))
...
filename <- "./figures/mobile_network_igraph_weekdays_cluster1.pdf"
mainStr <- "Mobile Data Network Visualization on Weekdays Cluster #1"
...
global.coeficient.cl1 <- transitivity(g, type="global")
local.coeficient.cl1 <- transitivity(g, type="local")
...
## For all degrees
pdf(file = "./figures/all_deg_dist_pl_weekdays_cl1.pdf")
op <- par(mfrow = c(2, 1))
options(scipen = 10)
## Histogram and cumulative distribution function
hist(degrees, freq = FALSE, xlab = "Degree k",
     main = "Histogram of All Degrees On Weekdays Cluster 1", breaks = 50, col = "gray")
mainStr <- "Degree Power-law Distribution On Weekdays Cluster 1"
...
# print("Fast Greedy community sizes:")
# print(sizes(fc))
community.cl2 <- data.frame(sizes(fc))
...
## Finally, plot and save the network
filename <- "./figures/mobile_network_igraph_weekdays_cluster2.pdf"
mainStr <- "Mobile Data Network Visualization on Weekdays Cluster #2"
plotNetwork(g, mainStr, filename, width=15, height=15)
...
global.coeficient.cl2 <- transitivity(g, type="global")
local.coeficient.cl2 <- transitivity(g, type="local")
...
## For all degrees
pdf(file = "./figures/all_deg_dist_pl_weekdays_cl2.pdf")
op <- par(mfrow = c(2, 1))
options(scipen = 10)
## Histogram and cumulative distribution function
hist(degrees, freq = FALSE, xlab = "Degree k",
     main = "Histogram of All Degrees On Weekdays Cluster 2", breaks = 50, col = "gray")
mainStr <- "Degree Power-law Distribution On Weekdays Cluster 2"
plot(x, cumy, log = "xy", xlab = "Degree k",
     ylab = expression(Pr(x) >= k), cex = 0.5, main = mainStr)
## Overlay the fitted distribution
startval <- cumy[pl$xmin]
