...
call.cluster <- read.csv(file = "./data/my_weekdays_caller_cluster.csv", header = TRUE)
...
my.call.data <- subset(my.cluster.call, date > 20080302) #change date
...
location.map <- location.map + ggtitle("Hourly Distribution of Call Events on Weekdays for Cluster 1")
...
ggsave(filename="./figures/my_call_bubbles_weekdays_hourly_cluster1.png", width=10, height=10) #change date
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Time Period Distribution of Call Events on Weekdays for Cluster 1")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
...
## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_weekdays_period_cluster1.png", width=10, height=10) #change date
...
g.map <- g.map + ggtitle("Spatial Call Graph on Weekdays for Cluster 1")
...
ggsave(filename="./figures/my_call_graph_weekdays_cluster1.png", width=10, height=10)
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Hourly Distribution of Call Events on Weekdays for Cluster 2")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
## Split the plot into tiles (or facets), one for each hour
location.map <- location.map + facet_wrap(~hourType)
...
## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_weekdays_hourly_cluster2.png", width=10, height=10) #change date
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Time Period Distribution of Call Events on Weekdays for Cluster 2")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
## Split the plot into tiles (or facets), one for each date
location.map <- location.map + facet_wrap(~periodZone)
...
## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_weekdays_period_cluster2.png", width=10, height=10) #change date
...
g.map <- g.map + geom_edgeset(aes(x = Longitude, y = Latitude, size=weight),
                              graph=g, color="blue")
## Add title and theme (optional)
g.map <- g.map + ggtitle("Spatial Call Graph on Weekdays for Cluster 2")
g.map <- g.map + fivethirtyeight_theme()
## Disable any legend
g.map <- g.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Display the map
...
## Save the plot to disk
ggsave(filename="./figures/my_call_graph_weekdays_cluster2.png", width=10, height=10)
## Saving Mapping Network Edges Ends
