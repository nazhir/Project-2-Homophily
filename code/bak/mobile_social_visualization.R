...
social.data <- read.csv(file="./data/my_social_weekend.csv")

subset.caller.data <- subset(call.data, caller_id %in% social.data$call_num) #
subset.callee.data <- subset(call.data, callee_id %in% social.data$call_num)

my.social.call <- rbind(subset.caller.data,subset.callee.data)

## Match each cell_id with its corresponding rowIndex of the coordinate table
progress.bar <- create_progress_bar("text")
progress.bar$init(nrow(my.social.call))
rowIndices <- vector() # a vector of row indices
nMatches <- 0 # count the number of matches
for(i in 1:nrow(my.social.call)) {
  ## Have to convert numeric to string in order to do lookup
  cell_id <- toString(my.social.call$cell_id[i])
  if(nchar(cell_id) > 0) {
...
my.call.data <- my.social.call[!is.na(rowIndices), ]
...
location.map <- location.map + ggtitle("Hourly Distribution of Call Events for Weekend Social Group")
...
ggsave(filename="./figures/my_call_bubbles_hourly_social_weekend.png", width=10, height=10) #change date

# buble plot ends

# spatial network edges begin
...
g.map <- g.map + ggtitle("Spatial Call Graph for Social Groups Weekend")
...
ggsave(filename="./figures/my_social_call_graph_weekend.png", width=10, height=10)
## Saving Mapping Network Edges Ends

# weekdays data

social.data <- read.csv(file="./data/my_social_weekdays.csv")
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Hourly Distribution of Call Events for Weekdays Social Group")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
## Split the plot into tiles (or facets), one for each hour
location.map <- location.map + facet_wrap(~hourType)
...
## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_hourly_social_weekdays.png", width=10, height=10) #change date
...
g.map <- g.map + geom_edgeset(aes(x = Longitude, y = Latitude, size=weight),
                              graph=g, color="blue")
## Add title and theme (optional)
g.map <- g.map + ggtitle("Spatial Call Graph for Social Groups Weekdays")
g.map <- g.map + fivethirtyeight_theme()
## Disable any legend
g.map <- g.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Display the map
...
## Save the plot to disk
ggsave(filename="./figures/my_social_call_graph_weekdays.png", width=10, height=10)
## Saving Mapping Network Edges Ends
