...
pdf(file = "./figures/mobile_cl_tree_Weekdays.pdf")
mainStr <- "Sequence Hierarchical Clustering on Weekdays"
...
pdf(file = "./figures/mobile_trajec_cl_Weekdays.pdf", width=20, height=10)
seqdplot(trajec.seq, group = trajec.cl, border = NA, title="Cluster on Weekdays", withlegend="auto")
dev.off()
...
save(caller_id.cluster, file="./data/weekdays_caller_id_cluster_mapping.RData")
...
cluster.map <- cluster.map + ggtitle("Clusters of Call Events over the Weekdays")
...
ggsave(filename="./figures/my_clustered_bubbles_weekdays.png", width=10, height=10)
...
write.csv(caller.id.cluster.map, file="./data/my_weekdays_caller_cluster.csv", row.names=FALSE)
