...
pdf(file = "./figures/mobile_cl_tree_80_100.pdf")
mainStr <- "Sequence Hierarchical Clustering for 80_100"
...
seqdplot(trajec.seq, group = trajec.cl, border = NA, title="Cluster for 80-100", withlegend="auto")
dev.off()
...
save(caller_id.cluster, file="./data/80_100_caller_id_cluster_mapping.RData")
...
cluster.map <- cluster.map + ggtitle("Clusters of Call Events for 80-100")
...
ggsave(filename="./figures/my_clustered_bubbles_80_100.png", width=10, height=10)
...
write.csv(caller.id.cluster.map, file="./data/my_80_100_caller_cluster.csv", row.names=FALSE)