...
my.call.data <- subset(my.call.data, date < 20080302)
...
location.map <- location.map + ggtitle("Hourly Distribution of Call Events for Saturday March, 1 2008")
...
ggsave(filename="./figures/my_call_bubbles_hourly_20080301.png", width=10, height=10) #change date

## Aggregate all the call records by time Period and find the locations of the cell towers
## and the frequency of the calls made from each.
aggregate.location <- aggregateLocationByPeriod(my.call.data,cell_id.coord.rowIndex, cell.towers)

period <- vector()
progress.bar <- create_progress_bar("text")
progress.bar$init(nrow(aggregate.location))
for(i in 1:nrow(aggregate.location)) {
  temp <- aggregate.location$period[i]
  
  if(temp == "1") {
    period[i] <- "01. 00.00 AM - 05.59 AM"
  } else if (temp == "2") {
    period[i] <- "02. 06.00 AM - 11.59 AM"
  } else if (temp == "3") {
    period[i] <- "03. 12.00 PM - 17.59 PM"
  } else if (temp == "4") {
    period[i] <- "04. 18.00 PM - 23.59 PM"
  } 
  
  progress.bar$step()
}
aggregate.location$periodZone <- period
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Time Period Distribution of Call Events for Saturday March, 1 2008")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
## Split the plot into tiles (or facets), one for each date
location.map <- location.map + facet_wrap(~periodZone)
## Display the plot (or else it won't show)
print(location.map)

## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_period_20080301.png", width=10, height=10) #change date

## creating call bubble plot end

## Start Mapping Network Edges
## Here, we plot the population graph for all the calls made during the whole weekend.
...
g.map <- g.map + ggtitle("Spatial Call Graph for Saturday March, 1 2008")
...
ggsave(filename="./figures/my_call_graph_20080301.png", width=10, height=10)
...
my.call.data <- subset(my.call.data, date > 20080301) #change date
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Hourly Distribution of Call Events for Sunday March, 2 2008")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
## Split the plot into tiles (or facets), one for each hour
location.map <- location.map + facet_wrap(~hourType)
...
## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_hourly_20080302.png", width=10, height=10) #change date
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Time Period Distribution of Call Events for Sunday March, 2 2008")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
## Split the plot into tiles (or facets), one for each date
location.map <- location.map + facet_wrap(~periodZone)
...
## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_period_20080302.png", width=10, height=10) #change date
...
g.map <- g.map + geom_edgeset(aes(x = Longitude, y = Latitude, size=weight),
                              graph=g, color="blue")
## Add title and theme (optional)
g.map <- g.map + ggtitle("Spatial Call Graph for Sunday March, 2 2008")
g.map <- g.map + fivethirtyeight_theme()
## Disable any legend
g.map <- g.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Display the map
...
## Save the plot to disk
ggsave(filename="./figures/my_call_graph_20080302.png", width=10, height=10)
## Saving Mapping Network Edges Ends
...
## Save the plot to disk
ggsave(filename="./figures/my_call_graph_200803.png", width=10, height=10)
## Saving Mapping Network Edges Ends

## Start Mapping Network Edges for weekend and weekdays
## Here, we plot the population graph for all the calls made during the whole weekend.
## First, construct a data frame that represents the weighted relationships between nodes.
## Each node is a caller and/or callee, whose location is mapped to a cell_id.
...
g.map <- g.map + geom_edgeset(aes(x = Longitude, y = Latitude, size=weight),
                              graph=g, color="blue")
## Add title and theme (optional)
g.map <- g.map + ggtitle("Spatial Call Graph for Weekend Duration")
g.map <- g.map + fivethirtyeight_theme()
## Disable any legend
g.map <- g.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Display the map
...
## Save the plot to disk
ggsave(filename="./figures/my_call_graph_200803_weekend.png", width=10, height=10)
...
g.map <- g.map + geom_edgeset(aes(x = Longitude, y = Latitude, size=weight),
                              graph=g, color="blue")
## Add title and theme (optional)
g.map <- g.map + ggtitle("Spatial Call Graph for Weekdays Duration")
g.map <- g.map + fivethirtyeight_theme()
## Disable any legend
g.map <- g.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Display the map
...
## Save the plot to disk
ggsave(filename="./figures/my_call_graph_200803_weekdays.png", width=10, height=10)
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Hourly Distribution of Call Events for Weekend")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
## Split the plot into tiles (or facets), one for each hour
location.map <- location.map + facet_wrap(~hourType)
...
## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_hourly_weekend.png", width=10, height=10) #change date
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Time Period Distribution of Call Events for Weekend")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
## Split the plot into tiles (or facets), one for each date
location.map <- location.map + facet_wrap(~periodZone)
...
## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_period_weekend.png", width=10, height=10) #change date
...
my.call.data <- subset(my.call.data, date > 20080302) #change date
my.call.data <- subset(my.call.data, date < 20080309)
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Hourly Distribution of Call Events for Weekdays")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
## Split the plot into tiles (or facets), one for each hour
location.map <- location.map + facet_wrap(~hourType)
...
## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_hourly_weekdays.png", width=10, height=10) #change date
...
location.map <- location.map + guides(fill=FALSE, alpha=FALSE, size=FALSE)
## Add a title to the plot
location.map <- location.map + ggtitle("Time Period Distribution of Call Events for Weekdays")
## Add a fancy theme to it (this step is optional)
location.map <- location.map + fivethirtyeight_theme()
## Split the plot into tiles (or facets), one for each date
location.map <- location.map + facet_wrap(~periodZone)
...
## Save the plot on disk
ggsave(filename="./figures/my_call_bubbles_period_weekdays.png", width=10, height=10) #change date
