...
load("./data/userId_map_reverse.RData")
...
weightedEdges <- read.csv(file = "./data/weighted_edges.csv", header = TRUE)

call.data <- read.csv(file = "./data/my_call_data_90_100_500.csv", header = TRUE)
...
edges <- data.frame(caller_id = subset.call.data$caller_id,callee_id = subset.call.data$callee_id)
