aggregateLocationByTimeZone <- function(call.data, cell_id.coord.rowIndex, cell.towers) {
  ## This function aggregates the retrieved call data by each hour.
  ...
  timezones <- names(table(call.data$timezone)) # get all the dates
  aggregate.data <- data.frame() # master data frame to store the records
  
  for(i in 1:length(timezones)) { # for each timezone
    ## Subset the data by timezone
    subset.call.data <- subset(call.data, timezone == timezones[i])
    ...
    timezone.aggregate.data <- data.frame(cell_id = cellIds, freq = cellId.freq,
                                      longitude = longitude, latitude = latitude,
                                      timezone = rep(timezones[i], counter))
    ## Combine each date with the master data frame
    aggregate.data <- rbind(aggregate.data, timezone.aggregate.data)
  }
