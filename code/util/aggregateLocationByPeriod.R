aggregateLocationByPeriod <- function(call.data, cell_id.coord.rowIndex, cell.towers) {
  ...
  periods <- names(table(call.data$period)) # get all the dates
  aggregate.data <- data.frame() # master data frame to store the records
  
  for(i in 1:length(periods)) { # for each timezone
    ## Subset the data by timezone
    subset.call.data <- subset(call.data, period == periods[i])
    ...
    period.aggregate.data <- data.frame(cell_id = cellIds, freq = cellId.freq,
                                          longitude = longitude, latitude = latitude,
                                          period = rep(periods[i], counter))
    ## Combine each date with the master data frame
    aggregate.data <- rbind(aggregate.data, period.aggregate.data)
  }
