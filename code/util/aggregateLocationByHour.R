aggregateLocationByHour <- function(call.data, cell_id.coord.rowIndex, cell.towers) {
  ...
  hours <- names(table(call.data$hour)) # get all the dates
  aggregate.data <- data.frame() # master data frame to store the records
  
  for(i in 1:length(hours)) { # for each date
    ## Subset the data by date
    subset.call.data <- subset(call.data, hour == hours[i])
    ...
    hour.aggregate.data <- data.frame(cell_id = cellIds, freq = cellId.freq,
                                      longitude = longitude, latitude = latitude,
                                      hour = rep(hours[i], counter))
    ## Combine each date with the master data frame
    aggregate.data <- rbind(aggregate.data, hour.aggregate.data)
  }
