...
pdf(file = "./figures/mobile_cl_tree_100_300.pdf")
mainStr <- "Sequence Hierarchical Clustering for 100_300"
...
pdf(file = "./figures/mobile_trajec_cl_80_100.pdf", width=20, height=10)
seqdplot(trajec.seq, group = trajec.cl, border = NA, title="Cluster for 100-300", withlegend="auto")
dev.off()
...
save(caller_id.cluster, file="./data/100_300_caller_id_cluster_mapping.RData")
...
cluster.map <- cluster.map + ggtitle("Clusters of Call Events for 100-300")
...
ggsave(filename="./figures/my_clustered_bubbles_100_300.png", width=10, height=10)
...
write.csv(caller.id.cluster.map, file="./data/my_100_300_caller_cluster.csv", row.names=FALSE)